...
#     # Search for experience pattern 1
#     match = re.search(experience_pattern_1, resume_text_no_phone, re.IGNORECASE)
#     if match:
#         years = float(match.group(1))
...
#     # Search for experience pattern 2
#     match = re.search(experience_pattern_2, resume_text_no_phone, re.IGNORECASE)
#     if match:
...
#     # Search for experience pattern 3
#     match = re.search(experience_pattern_3, resume_text_no_phone, re.IGNORECASE)
#     if match:
#         years = float(match.group(1))
...
#     # Search for experience pattern 4
#     match = re.search(experience_pattern_4, resume_text_no_phone, re.IGNORECASE)
#     if match:
#         years = float(match.group(1))
#         total_months = int(years * 12)
#         if total_months < 600:  # Assuming 50 years of experience as a reasonable upper limit
...
#     # Search for experience pattern 5
#     match = re.search(experience_pattern_5, resume_text_no_phone, re.IGNORECASE)
#     if match:
#         years = float(match.group(1))
#         total_months = int(years * 12)
#         if total_months < 600:  # Assuming 50 years of experience as a reasonable upper limit
...
#     candidate_experience_months = extract_experience_from_resume(resume_text)
#     candidate_experience_years = candidate_experience_months / 12  # Convert months to years
...
#     # Format candidate_experience as "2 years 0 months" and convert it to float like 2.0
#     candidate_experience_formatted = f"{math.floor(candidate_experience_years)}.{candidate_experience_months % 12}"
...
#     'Financial Analysis', 'Customer Relationship Management (CRM)',
#     'Quality Management', 'Supply Chain Management', 'Logistics', 'Health and Safety', 'Public Relations',
#     'Social Media Management', 'Content Creation', 'Graphic Design', 'Video Production', 'Photography',
#     'Editing', 'UI/UX Design', 'Web Design', 'Architecture', 'Interior Design', 'Fashion Design',
#     'Legal Knowledge', 'Medical Knowledge', 'Teaching', 'Education', 'Economics', 'Psychology'
# ]
...
# def extract_text_from_pdf(file_binary):
#     text = ''
#     reader = PyPDF2.PdfFileReader(file_binary)
#     for page_num in range(reader.numPages):
#         text += reader.getPage(page_num).extract_text()
#     return text

# def extract_text_from_docx(file_binary):
#     doc = docx.Document(file_binary)
#     text = ''
#     for para in doc.paragraphs:
#         text += para.text
#     return text
...
# def combine_skills_with_sub_categories(skills):
#     combined_skills = set(skills)
#     for skill in skills:
#         sub_skills = generate_sub_skills(skill)
#         combined_skills.update(sub_skills)
#     return list(combined_skills)

# def extract_skills_from_resume(resume_text, skills_list):
#     resume_text = preprocess_text(resume_text)
#     skills_found = []
#     all_skills = it_skills + non_it_skills  # Combine both lists for matching
...
#         'client': job_post.client,
#         'detailed_jd': job_post.detailed_jd if job_post.detailed_jd else "",
#         'skills': job_post.skills if job_post.skills else [],
#         'experience_min': job_post.experience_min,
#         'experience_max': job_post.experience_max
#     }
#     return job_details
...
#     combined_skills = combine_skills_with_sub_categories(job_details['skills'])
#     matched_skills = extract_skills_from_resume(resume_text, combined_skills)
#     total_skills_from_job = len(combined_skills)
#     matched_skills_count = len(set(combined_skills).intersection(matched_skills))
#     skill_match_percentage = (matched_skills_count / total_skills_from_job) * 100 if total_skills_from_job > 0 else 0
...
#     reader = PyPDF2.PdfFileReader(pdf_binary)
#     text = ''
#     for page_num in range(reader.numPages):
#         text += reader.getPage(page_num).extract_text()
#     return text
...
# def calculate_skill_match_percentage(skills_list, text):
#     text = preprocess_text(text)
#     skill_matches = sum(1 for skill in skills_list if re.search(r'\b' + re.escape(skill.lower()) + r'\b', text))
#     total_skills = len(skills_list)
    
#     match_percentage = (skill_matches / total_skills) * 100 if total_skills > 0 else 0
...
#     job_details = {
#         'client': job_post.client,
#         'detailed_jd': job_post.detailed_jd if job_post.detailed_jd else "",  # Fetch from detailed_jd
#         'skills': job_post.skills if job_post.skills else "",
#         'experience_min': job_post.experience_min,
#         'experience_max': job_post.experience_max
#     }
...
# def extract_experience_from_resume(resume_text):
#     experience_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:years?|yrs?)\s*experience', resume_text, re.IGNORECASE)
#     if experience_match:
#         return float(experience_match.group(1))
#     return 0
...
#     if not job_id or not resume_base64 or not user_id:
#         return jsonify({'status': 'error', 'message': 'job_id, resume, and user_id are required'}), 400
...
#             return jsonify({'status': 'error', 'message': 'User does not have permission to perform this action'}), 401
...
#             resume_text = extract_text_from_resume(resume_binary)
#         except Exception:
#             return jsonify({'status': 'error', 'message': 'Error decoding base64 resume file'}), 400

#         # Extract candidate's experience from resume text
...
#         # Convert experience fields to floats
#         experience_min = float(job_details['experience_min'])
#         experience_max = float(job_details['experience_max'])

#         # Extract and compare qualifications from job details
#         job_qualifications = re.findall(r'qualification\s*:\s*(.+?)\s*roles', job_details['detailed_jd'], re.IGNORECASE)
#         job_qualifications = job_qualifications[0] if job_qualifications else ""
        
#         # Extract skills from job details and resume
#         job_skills = job_details['skills'].split(',')
#         resume_skills = preprocess_text(resume_text).split()

#         # Calculate skill match percentage for IT skills against resume skills
#         it_skill_match_percentage = calculate_skill_match_percentage(it_skills, " ".join(resume_skills))

#         # Calculate skill match percentage for Non-IT skills against resume skills
#         non_it_skill_match_percentage = calculate_skill_match_percentage(non_it_skills, " ".join(resume_skills))

#         # Calculate overall match percentage with job requirements
#         match_percentage = calculate_skill_match_percentage(job_skills, " ".join(resume_skills))

#         # Check if candidate's experience matches job requirements
#         experience_match = experience_min <= candidate_experience <= experience_max

#         return jsonify({
#             'status': 'success',
#             'match_percentage': match_percentage,
#             'it_skill_match_percentage': it_skill_match_percentage,
#             'non_it_skill_match_percentage': non_it_skill_match_percentage,
#             'experience_match': experience_match,
#             'candidate_experience': candidate_experience
#         }), 200
#     except Exception as e:
#         print(e)
#         return jsonify({'status': 'error', 'message': f'Failed to check resume match: {str(e)}'}), 500
...
# def calculate_skill_match_percentage(skills_list, resume_text):
#     resume_text = preprocess_text(resume_text)
#     skill_matches = sum(1 for skill in skills_list if re.search(r'\b' + re.escape(skill.lower()) + r'\b', resume_text))
#     total_skills = len(skills_list)
...
#         # Calculate skill match percentage for IT skills
#         it_skill_match_percentage = calculate_skill_match_percentage(it_skills, resume_text)

#         # Calculate skill match percentage for Non-IT skills
#         non_it_skill_match_percentage = calculate_skill_match_percentage(non_it_skills, resume_text)

#         # Calculate overall match percentage with job requirements
#         job_requirements = f"{job_details['detailed_jd']} {job_details['skills']}"
#         match_percentage = calculate_skill_match_percentage(job_requirements.split(), resume_text)
...
# def extract_text_from_pdf(pdf_binary):
#     reader = PdfFileReader(pdf_binary)
#     text = ''
#     for page_num in range(reader.numPages):
...
# def extract_text_from_resume(file_binary):
#     # Attempt to extract as PDF
#     try:
#         return extract_text_from_pdf(file_binary)
#     except PyPDF2.utils.PdfReadError:
#         pass
    
#     # Attempt to extract as DOCX
#     try:
#         return extract_text_from_docx(file_binary)
#     except Exception:
#         raise ValueError("Unsupported file format")
...
#     job_words = job_requirements.split()
...
#     match_count = sum(min(job_counter[word], resume_counter[word]) for word in common_words)
    
#     total_job_words = sum(job_counter.values())
#     match_percentage = (match_count / total_job_words) * 100 if total_job_words > 0 else 0
...
#     job_post = JobPost.query.filter_by(id=job_id).first()
#     job_details = {
#         'qualifications': job_post.detailed_jd if job_post.detailed_jd else "",
#         'skills': job_post.skills if job_post.skills else "",
#         'experience_min': job_post.experience_min,
#         'experience_max': job_post.experience_max
#     }
...
#     resume_base64 = data.get('resume')
#     user_id = data.get('user_id')
#     resume_as_string = data.get('resume_as_string', False)
...
#         # Combine qualifications and skills for matching
#         job_requirements = f"{job_details['qualifications']} {job_details['skills']}"
...
#     job_post = JobPost.query.filter_by(id=job_id).first()
#     job_details = {
#         'qualifications': job_post.detailed_jd,
...
#     if not job_id or not resume_base64 or not user_id:
#         return jsonify({'status': 'error', 'message': 'job_id, resume_base64, and user_id are required'}), 400
...
#         # Combine qualifications and skills for matching
#         job_requirements = job_details['qualifications'] + ' ' + job_details['skills']
#         match_percentage = calculate_match_percentage(job_requirements, resume_text)
...
def change_password():
    ...
    if user.user_type == 'management':
        return jsonify({'status': 'success', 'message': 'Password changed successfully for management {user.username}.'})
    elif user.user_type == 'recruiter':
        return jsonify({'status': 'success', 'message': 'Password changed successfully for recruiter{user.username}.'})
    else:
        return jsonify({'status': 'success', 'message': 'Password changed successfully.'})
...
#     overall_total_candidates = 0
#     overall_unsuccessful_closures = 0
#     overall_total_screening_candidates = 0
#     overall_total_onboarded_positions = 0
#     overall_total_candidates_in_progress = 0
...
#         # Increment overall total candidates count
#         overall_total_candidates += total_candidates

#         for candidate in candidates:
#             if candidate.status == 'SCREENING':
#                 total_screening_candidates += 1
#                 overall_total_screening_candidates += 1
#             elif candidate.status == 'ON-BOARDED':
#                 count_of_onboarded_positions += 1
#                 overall_total_onboarded_positions += 1
...
#                 unsuccessful_closures += 1
#                 overall_unsuccessful_closures += 1
...
#         # Calculate candidates in progress
#         overall_total_candidates_in_progress = overall_total_candidates - (overall_total_onboarded_positions + overall_total_screening_candidates + overall_unsuccessful_closures)
...
#             'percentage_onboarded': percentage_onboarded,
#             'total_candidates_count': total_candidates,
#             'candidates_in_progress': candidates_in_progress  # New field
#         }

#         result[recruiter_name] = recruiter_data

#     # Overall counts for all recruiters
#     overall_counts = {
#         'overall_total_candidates': overall_total_candidates,
#         'overall_unsuccessful_closures': overall_unsuccessful_closures,
#         'overall_total_screening_candidates': overall_total_screening_candidates,
#         'overall_total_onboarded_positions': overall_total_onboarded_positions,
#         'overall_total_candidates_in_progress':overall_total_candidates_in_progress
        
#     }

#     response_data = {
#         'recruiters_data': result,
